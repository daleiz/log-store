...
import System.IO (hPutStr, hSetBuffering, stdout)
...
main = hspec $
  describe "Basic Functionality" $
    do
      it "append speed test" $
        withLogStore
          (Config {rootDbPath = "db-temp"})
          ( do
              logHandle <-
                open
                  "log"
                  defaultOpenOptions {writeMode = True, createIfMissing = True}
              appendForever logHandle 0 0
              return "success"
          )
...
appendForever lh bytes duration = do
  ...
  let bytes' = bytes + 128 * 4096
  let duration' = duration + deltaT
  if duration' >= 1
    then do
      let speed = fromIntegral bytes' / duration' / duration' / 1024 / 1024
      liftIO $
        putStrLn $
          "\r=> " <> "append speed: "
            <> showFFloat (Just 2) speed " MB/s"
      appendForever lh 0 0
    else appendForever lh bytes' duration'
